...
const torus = new THREE.Mesh(new THREE.TorusGeometry(3.05, 0.34, 14, 96), torusMaterial);
reactor.add(torus);
...
const core = new THREE.Mesh(new THREE.SphereGeometry(1.02, 48, 32), coreMaterial);
reactor.add(core);
...
const wantedColor = new THREE.Color(0x22d3ee);
...
function setMode(mode, detail = '') {
  ...
  wantedColor.setHex(style.color);
...
function attachPlaybackAnalyser() {
  try {
    ...
    if (!playbackAnalyser) {
      ...
      playbackAnalyser.connect(analyser || gain);
    }
